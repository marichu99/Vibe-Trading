...
def _journal_reconcile_closed(symbol: str, connection: str) -> None:
    ...
    if updates_by_ticket:
        # Re-read the journal right before writing and merge by ticket into
        # that fresh copy, rather than writing back the `entries` snapshot
        # taken at the top -- get_positions/get_open_orders are real broker
        # round trips, so that snapshot is stale by now. --status runs this
        # same reconciliation with no lock while the live loop can
        # concurrently append a brand-new open entry via _journal_record_open;
        # writing back the stale snapshot would silently drop that new entry.
        fresh = _read_journal()
...
def _resolve_fill_price(trade: dict, placed_order: dict) -> float | None:
    ...
    _post_trade_reward_risk_check's own position-modification match: with a
    target's max_stack > 1, more than one position can legitimately be open
    on this symbol, and "first match by symbol+magic" can silently return an
    OLDER position's entry price instead of the one that just filled.
    """
